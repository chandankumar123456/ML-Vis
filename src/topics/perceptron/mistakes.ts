...
export const perceptronMistakes: Mistake[] = [
  ...
  },
  {
    ...
    pattern: 'Reading (R/γ)² as a performance prediction or tight runtime estimate',
    example: '\\left(\\frac{R}{\\gamma}\\right)^2 \\approx 2261 \\;\\text{vs measured 4 updates}',
    whyWrong: 'Novikoff\'s bound is a worst-case guarantee, and for practical draws it is extremely loose: on the default seed it evaluates to ≈ 2261 updates while the run takes 4 (seed 7: 23). The bound\'s real content is QUALITATIVE — separability ⇒ finitely many updates — plus the honest warning that tiny margins γ make the bound huge. Using it to predict runtime or to compare algorithms misreads the mathematics.',
    gateTrap: false,
    relatedConcept: 'perceptron-convergence-bound',
  },
  {
    id: 'perceptron-sigmoid-probability',
    pattern: 'Applying a sigmoid σ(w·x+b) and thresholding at 0.5, thinking the perceptron outputs probabilities',
    example: '\\sigma(w \\cdot x + b) \\ge 0.5 \\;\\Rightarrow\\; \\text{class 1 — as if } P(y{=}1\\mid x) \\text{ existed}',
    whyWrong: 'The classic perceptron outputs a HARD sign decision — there is no σ, no probability, and no confidence calibration anywhere in the rule; the score\'s magnitude carries no probabilistic meaning. Sigmoid outputs are logistic regression\'s job (it optimizes a proper probabilistic loss); bolting σ onto the perceptron changes neither how it trains nor what it claims to output.',
    gateTrap: true,
    relatedConcept: 'perceptron-update',
  },
  {
    id: 'perceptron-update-every-point',
    pattern: 'Applying the update w ← w + η·yᵢ·xᵢ on EVERY point instead of only on mistakes',
    example: '\\text{update on all } (x_i, y_i) \\text{ every pass } \\;\\Rightarrow\\; \\text{the boundary drifts past already-correct points}',
    whyWrong: 'The classic rule updates ONLY on mistakes (yᵢ·sᵢ ≤ 0); correct points are examined and skipped because an update on a correct point pushes the boundary AWAY from correctness again. Updating on every point turns the perceptron into a different, unregularized, drifting algorithm: the boundary keeps moving after points are already correct, and the clean convergence guarantee (Novikoff) no longer applies.',
    gateTrap: true,
    relatedConcept: 'perceptron-update',
  },
];