...
export const mleFormulas: Formula[] = [
  {
    ...
    failureCases: ['Underflow: for n = 1000 the product of 1000 factors below 1 underflows — measured L(0.707) = 2.1465e−263 and L(0.9) = 0 in double precision, while ℓ(0.9) = −749.147 stays finite', 'L is a product, so a single zero-probability factor makes the whole likelihood 0'],
    derivesFrom: [],
  ...
  },
  {
    ...
    failureCases: ['Working with L directly loses underflow: at n=1000, L(0.9) = 0 exactly (product of 0.9 and 0.1 powers), while ℓ(0.9) = −749.147 remains perfectly representable', 'ℓ can be very negative (large n); comparing ℓ values across DIFFERENT n is meaningless (each sample contributes a log-probability term)'],
    derivesFrom: ['mle-likelihood'],
    derivationIds: ['mle-likelihood-derivation'],
    connections: ['Score function', 'Cross-entropy loss', 'Likelihood'],
    whyWorks: 'The log turns the product into a sum — numerically stable and analytically easier (derivatives of sums are sums of derivatives). Because ln is strictly increasing, the SAME θ maximizes L and ℓ: the measured grid at n=10 shows both L(p) and ℓ(p) peaking at p = 0.8 = k/n. The average −ℓ(θ̂)/n is the nllPerSample loss curve this topic plots (lossMetricKey), which converges upward toward the true entropy as n grows: 0.5004 → 0.6048 for the coin, with H(0.7) = 0.610864 the limit.',
  },
  {
    id: 'mle-score',
  ...
  },
  {
    ...
    whyWorks: 'The MLE is consistent: it converges in probability to the true θ and is asymptotically normal with variance 1/I (efficiency). The seeded sweep demonstrates the convergence: p̂ error 0.1 → 0.04 → 0.007 over n = 10 → 100 → 1000; gaussian μ̂ error 0.940 → 0.071; linear slope error 0.095 → 0.012. The nllPerSample curve converges upward toward the true entropy H(0.7) = 0.610864 as n grows.',
  },
  {
    id: 'mle-invariance',
  ...
  },
  {
    ...
    whyWorks: 'Maximizing ℓ is exactly minimizing average negative log-likelihood — the same objective deep-learning frameworks call cross-entropy. The loss-curve view plots nllPerSample = −ℓ(θ̂)/n: on the coin it ascends 0.5004 → 0.6048 toward the true entropy H(p) = 0.610864, the theoretical floor. This is the "CE loss = negative log-likelihood" link the plan calls out.',
  },
];